...
- [x] **Visually appealing colors and layout. No overflowing elements.** - Nice blue ocean theme
- [x] **Use of a CSS framework** - I used the CSS framework
- [x] **All visual elements styled using CSS** - All visual elements styled using CSS
- [x] **Responsive to window resizing using flexbox and/or grid display** - The window resizes using flex
- [x] **Use of a imported font** - I imported and used the font Inter
- [x] **Use of different types of selectors including element, class, ID, and pseudo selectors** - I used different selectors sparingly, opting to use mainly the css file, however I did use all of the differen't selectors in the html files at least once.

## 🚀 React part 1: Routing deliverable

For this deliverable I did the following. I checked the box `[x]` and added a description for things I completed.

- [x] **Bundled using Vite** - I successfully configured Vite to bundle the React project.
- [x] **Components** - I refactored the application into reusable React components.
- [x] **Router** - I implemented React Router to handle navigation between pages within the application.
...
- [x] **Backend listens for WebSocket connection** - I set up a WebSocket server on the backend and attached it to the same HTTP server as Express.
- [x] **Frontend makes WebSocket connection** - The game page opens a WebSocket connection when it loads.
- [x] **Data sent over WebSocket connection** - The frontend sends game start messages and the backend broadcasts game start and score update events.
- [x] **WebSocket data displayed** - Broadcast messages are shown in the live game feed on the page.
- [x] **Application is fully functional** - It's finally done
